...
export default function Chat() {
    ...
    useEffect(()=>{
        ...
        async function loadConversations(){
            ...
            let conversations = []
            const conversationsSnapshot = await firebase.firestore().collection('Conversations').get()
            conversationsSnapshot.forEach(async conversation=>{
                ...
                if(peopleInvolved.includes(user.uid)){
                    let otherUserID = peopleInvolved[0] == user.uid ? peopleInvolved[1] : peopleInvolved[0]
                    let conversationObject = {
                        ...
                        messages: Array.from(await conversation.ref.collection("Messages").get())
        ...
        async function loadProfiles(personID){
            ...
            try{
                return await firebase.storage().ref('UsersProfiles/' + personID).getDownloadURL()
            }catch{
    ...
    return (
        <View style={styles.chat}>
            ...
            <View style={styles.inputView}>
                <TextInput style={styles.input} placeholder="Procure alguém"></TextInput>
            </View>
            ...
            <FlatList
                ...
                renderItem={({ item }) => {
                    return (
                        ...
                        <Person conversationObject={item}/>
